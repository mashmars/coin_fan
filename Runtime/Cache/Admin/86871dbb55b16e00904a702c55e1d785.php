...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a href="javascript:;" onclick="admin_add('添加菜单','admin_menu_add.html','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加菜单</a></span> <span class="r">共有数据：<strong>54</strong> 条</span> </div>
	<table class="table table-border table-bordered table-bg table-hover table-sort">
		<thead>
			...
			<tr class="text-c">
				...
				<th width="150">控制器/方法名称</th>
				<th width="250">菜单级别</th>
				<th width="200">操作</th>
			</tr>
		</thead>
		<tbody>
		<?php if(is_array($menu)): $i = 0; $__LIST__ = $menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				<td><?php echo ($vo["id"]); ?></td>
				<td style='text-align:left;'><?php echo (str_repeat('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;¦---',$vo["level"])); echo ($vo["title"]); ?></td>
				<td ><?php echo ($vo["name"]); ?></td>
				<td><?php if($vo["level"] == 0): ?>一级菜单<?php elseif($vo["level"] == 1): ?>二级菜单<?php elseif($vo["level"] == 2): ?>三级菜单<?php endif; ?></td>
				<td class="td-manage">
				<button type='button' class="btn btn-primary-outline radius" onclick="admin_edit('菜单编辑','<?php echo U('admin_menu_edit',array('id'=>$vo['id']));?>','1','800','500')"><i class="Hui-iconfont">&#xe6df;</i>编辑</button>
				<button type='button' class="btn btn-danger-outline radius" onclick="admin_del(this,'<?php echo ($vo['id']); ?>')"><i class="Hui-iconfont">&#xe6e2;删除</button>
				<!-- <button type='button' class="btn btn-success-outline radius" onclick="admin_edit('分配角色','admin_edit.html','1','800','500')"><i class="Hui-iconfont">&#xe600;分配菜单</button> -->
...
<script type="text/javascript" src="<?php echo (PUB_LIB); ?>laypage/1.2/laypage.js"></script>
...
function admin_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: "<?php echo U('auth/ajax_menu_del');?>",
			dataType: 'json',
			data:{id:id},
