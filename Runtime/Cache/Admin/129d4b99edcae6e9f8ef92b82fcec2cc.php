...
<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg ">
		...
		<tbody>
		<?php if(is_array($zhengshu)): $i = 0; $__LIST__ = $zhengshu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				
				<td><?php echo ($vo["id"]); ?></td>
				<td><?php echo ($vo["zsmc"]); ?></td>	
				<td class="td-manage">				
				<a style="text-decoration:none" class="btn btn-secondary-outline radius size-MINI" onclick="member_add('编辑证书信息','<?php echo U('user/XX',array('id'=>$vo['id']));?>','','310')"  title="添加证书信息">编辑</a>  
				<a style="text-decoration:none" class="btn btn-danger-outline radius size-MINI" onClick="del_zs(this,'<?php echo ($vo["id"]); ?>')" href="javascript:;" title="删除">删除</a>
				</td>
...
function del_zs(obj,id){
	layer.confirm('是否确认删除此信息,一经删除，无法恢复',function(){
		$.ajax({
			url:"<?php echo U('user/ajax_zhengshu_delete');?>",
			data:{id:id},
			type:"post",
			dataType:"json",
...
function del_reg(obj,id){
	layer.confirm('是否确认删除此信息,一经删除，无法恢复',function(){
		$.ajax({
			url:"<?php echo U('user/ajax_reg_delete');?>",
			data:{id:id},
			type:"post",
			dataType:"json",
			success:function(data){
