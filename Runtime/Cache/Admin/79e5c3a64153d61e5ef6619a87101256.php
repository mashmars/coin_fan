...
<div class="page-container">
	...
	<table class="table table-border table-bordered table-hover table-bg ">
		...
		<?php if(is_array($res)): $i = 0; $__LIST__ = $res;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				...
				<a style="text-decoration:none" class="btn btn-secondary-outline radius size-MINI" onClick="login(this,'<?php echo ($vo["id"]); ?>')" href="javascript:;" title="登入">登入</a>  
				</td>
...
function login(obj,id){
	$.ajax({
		url:"<?php echo U('user/ajax_yidi_login');?>",
		data:{id:id},
		type:"post",
		dataType:"json",
