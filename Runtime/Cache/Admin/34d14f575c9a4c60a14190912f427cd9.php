...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">	
		<a href="javascript:;" onclick="member_add('添加动态设置','<?php echo U('system/sys_dtfh_add');?>','','510')" class="btn btn-primary radius">
	...
	<table class="table table-border table-bordered table-hover table-bg ">
		...
		<?php if(is_array($res)): $i = 0; $__LIST__ = $res;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				...
				<td class="td-manage">				
				<a style="text-decoration:none" class="btn btn-secondary-outline radius size-MINI" onclick="member_add('编辑','<?php echo U('system/sys_dtfh_edit',array('id'=>$vo['id']));?>','','510')"   title="编辑">编辑</a>  
...
function del(obj,id){
	layer.confirm('是否确认删除,一经删除，无法恢复',function(){
		$.ajax({
			url:"<?php echo U('system/ajax_sys_dtfh_del');?>",
			data:{id:id},
			type:"post",
			dataType:"json",
