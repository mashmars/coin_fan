...
<div class="page-container">
	...
	<table class="table table-border table-bordered table-bg">
		...
		<tbody>
		<?php if(is_array($role)): $i = 0; $__LIST__ = $role;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				<td><?php echo ($vo["id"]); ?></td>
				<td><?php echo ($vo["title"]); ?></td>
				<td><a class='btn btn-success-outline radius' onclick="admin_edit('查看权限','<?php echo U('auth/admin_role_view',array('id'=>$vo['id']));?>','1','800','500')">点击查看</a></td>
				<td class="td-manage">
				<button type='button' class="btn btn-primary-outline radius" onclick="admin_edit('角色编辑','<?php echo U('auth/admin_role_edit',array('id'=>$vo['id']));?>','1','800','500')"><i class="Hui-iconfont">&#xe6df;</i>编辑</button>
				...
				<button type='button' class="btn btn-success-outline radius" onclick="admin_edit('分配权限','<?php echo U('auth/admin_role_grant',array('id'=>$vo['id']));?>','1','800','500')"><i class="Hui-iconfont">&#xe600;分配权限</button>
...
function admin_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: "<?php echo U('auth/ajax_role_del');?>",
			data:{id:id},
			dataType: 'json',
