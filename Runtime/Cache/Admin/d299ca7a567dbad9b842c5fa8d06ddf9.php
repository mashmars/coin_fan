...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 求职需求 <span class="c-gray en">&gt;</span> 求职需求 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
	 <span class="r">共有数据：<strong></strong> <?php echo ($count); ?>条</span>
	</div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover">
			...
			<?php if(is_array($res)): $i = 0; $__LIST__ = $res;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">					
					...
					<td class="text-l"><?php echo ($vo["phone"]); ?></td>	
					<td class="text-l"><?php echo ($vo["gangwei"]); ?></td>	
					<td><?php echo (date('Y-m-d H::s',$vo["createdate"])); ?></td>					
...
//是否显示控制
	...
	function gonggao_del(obj,id){ 
		layer.confirm('确认要删除吗？',function(index){
			$.ajax({
				type: "POST",
				url: "<?php echo U('news/ajax_job_del');?>",
				data:{id:id},
				dataType: "json",
				success: function(data){
