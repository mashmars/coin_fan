...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a href="javascript:;" onclick="admin_add('添加管理员','<?php echo U('auth/admin_add');?>','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加管理员</a></span> <span class="r">共有数据：<strong>54</strong> 条</span> </div>
	<table class="table table-border table-bordered table-bg">
		...
		<tbody>
		<?php if(is_array($admin)): $i = 0; $__LIST__ = $admin;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				<td><?php echo ($vo["id"]); ?></td>
				<td><?php echo ($vo["username"]); ?></td>
				<td><?php echo ($vo["title"]); ?></td>
				<td><?php echo ($vo["descript"]); ?></td>
				<td><?php echo (date('Y-m-d H:i:s',$vo["last_log_time"])); ?></td>
				<td><?php echo ($vo["last_log_ip"]); ?></td>
				<td class="td-manage">
				<button type='button' class="btn btn-primary-outline radius" onclick="admin_edit('管理员编辑','<?php echo U('auth/admin_edit',array('id'=>$vo['id']));?>','1','800','500')"><i class="Hui-iconfont">&#xe6df;</i>编辑</button>
